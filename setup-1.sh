...
# Remote: curl https://raw.githubusercontent.com/MrSimonEmms/z2h-k3s-guide/main/setup.sh | CMD=install bash
###
...
Z2H_NAMESPACE="${Z2H_NAMESPACE:-z2h}"
CONTEXT_NAME="${CONTEXT_NAME:-z2h-k3s}"
K3S_CHANNEL="${K3S_CHANNEL:-stable}"
REPO_RAW_URL="${REPO_RAW_URL:-https://raw.githubusercontent.com/MrSimonEmms/z2h-k3s-guide/main}"
...
function setup_monitoring() {
  if [ "${INSTALL_MONITORING}" = "true" ]; then
    ...
    kubectl create namespace "${Z2H_NAMESPACE}" || true
    ...
    helm upgrade \
      ...
      --reset-values \
      --set z2hNamespace="${Z2H_NAMESPACE}" \
      --wait \
      monitoring \
      z2h-monitoring
  else
...
function install() {
  echo "Installing z2h to k3s cluster"
...
EOF
    ...
    EXTRA_ARGS="--node-label=z2h.io/workload_meta=true --node-label=z2h.io/workload_ide=true --node-label=z2h.io/workload_workspace_services=true --node-label=z2h.io/workload_workspace_regular=true --node-label=z2h.io/workload_workspace_headless=true"
...
Your cloud infrastructure is ready to install z2h. Please visit
https://www.z2h.io/docs/self-hosted/latest/getting-started#step-4-install-z2h
for your next steps.
...
function uninstall() {
  echo "Uninstalling z2h from k3s cluster"

  if [ "${USE_REMOTE_REPO}" -eq 1 ]; then
    REPLY="y"
  else
    read -p "Are you sure you want to delete: z2h (y/N)?" -n 1 -r
    echo ""
  fi
